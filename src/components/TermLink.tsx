...
const linkClassesSoft = linkClasses + ' ' +[
	'bg-gray-700',
	'md:hover:bg-gray-500',
	'bg-opacity-80',
	'md:hover:bg-opacity-80',
].join(' ')
const linkClassesHard = linkClasses + ' ' + [
	'bg-purple-900',
...
const TermLink: React.FC<Props> = ({ children, to, soft }) => (
	<Link to={to} className={soft ? linkClassesSoft : linkClassesHard}>
		{children}
	</Link>
)
...
export const TermAction: React.FC<ActionProps> = ({ children, onClick, soft }) => (
	<a href='#play' className={soft ? linkClassesSoft : linkClassesHard} onClick={onClick}>
		{children}
	</a>
)
